...
// (small tweaks: smoother handoff + scroll-to-services chip)
// =============================
"use client"
    ...
    >
      ...
      <div className="absolute top-6 left-0 right-0 z-20 mx-auto flex max-w-6xl items-center justify-between px-6">
        ...
        <div className="hidden gap-6 md:flex text-zinc-400">
          <a href="#services" className="hover:text-zinc-200 transition">Servicios</a>
          <a href="#proyectos" className="hover:text-zinc-200 transition">Proyectos</a>
          <a href="#contact" className="hover:text-zinc-200 transition">Contacto</a>
        </div>
      </div>
      ...
      <div className="relative z-10 mx-auto grid max-w-7xl grid-cols-1 items-center gap-10 px-6 pt-28 md:grid-cols-2 md:gap-12 md:pt-40">
        ...
        <div>
          ...
          >
            Diseño <span className="bg-gradient-to-r from-white to-zinc-400 bg-clip-text text-transparent">personalizado</span> +
          ...
          >
            Construimos experiencias digitales a medida, rápidas y accesibles. El estándar Grimlo.
          </motion.p>
          ...
          >
            ...
            {/* Chip para ir a Paquetes (mejor fusión) */}
            <Link href="#services" className="group inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/5 px-4 py-3 text-sm text-white/80 hover:bg-white/10 transition">
              Ver paquetes
        ...
        <div className="relative h-[520px] md:h-[600px]">
          ...
          <ParallaxCard delay={0}   z={40} className="right-8 top-6 h-60 w-[75%] md:w-[70%]" label="Advanced Web" image="/ValeraVisionarias.gif" />
          <ParallaxCard delay={0.1} z={28} className="left-4 top-40 h-64 w-[70%] md:w-[66%]" label="E-commerce"     image="/UrbanBeat.gif" />
          <ParallaxCard delay={0.2} z={16} className="right-12 bottom-4 h-56 w-[62%] md:w-[58%]" label="Lite Web"       image="/GRILLIX.gif" />
        </div>
      </div>

      {/* Cue scroll convertido en enlace */}
      <Link href="#services" className="pointer-events-auto absolute bottom-6 left-1/2 z-10 -translate-x-1/2 text-xs text-zinc-500">
        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.6 }} className="flex items-center gap-2 hover:text-zinc-300">
          Desliza a paquetes
          <span className="h-4 w-4 rounded-full border border-zinc-700 overflow-hidden">
            <motion.span className="mx-auto mt-0.5 block h-2 w-0.5 rounded bg-zinc-400" animate={{ y: [1, 9, 1], opacity: [0.2, 1, 0.2] }} transition={{ repeat: Infinity, duration: 1.6, ease: "easeInOut" }} />
          </span>
        </motion.div>
      </Link>

      {/* Borde superior brillo + fade inferior para fusión con Packages */}
...
function ParallaxCard({ className = "", z = 20, delay = 0, image = "", label = "Proyecto" }: { className?: string; z?: number; delay?: number; image?: string; label?: string }) {
  return (
    <motion.div
    ...
    >
      <div className="relative overflow-hidden rounded-2xl">
        <img src={image || "data:image/svg+xml;charset=UTF-8,%3Csvg xmlns='http://www.w3.org/2000/svg' width='1200' height='800'%3E%3Crect width='100%25' height='100%25' fill='%23121212'/%3E%3C/ svg%3E"} alt={label} className="h-full w-full object-cover" loading="eager" />
